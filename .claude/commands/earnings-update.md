...
1. **Reconcile the actuals.** Reliability-tag and recompute derived figures;
   ...
   needed). A figure that doesn't tie out is not usable.
2. **Compare each swing input vs the thesis** and update the table (latest actual
   + as-of + on-track?). Decompose the revenue move into volume × price as in 06b
   — a beat driven by price/take rate is more fragile than one driven by volume.
...
7. **Re-model?** Only if a falsifier tripped or a swing input materially deviated —
   ...
   pipeline. Otherwise the tracker update IS the whole job.
